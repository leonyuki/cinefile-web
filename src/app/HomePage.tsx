...
import heroImage from '../../imports/image-7.png';
      ...
      <section className="max-w-7xl mx-auto px-6 sm:px-12 py-16 sm:py-20">
        <div className="grid md:grid-cols-2 gap-12 items-start">
          ...
          <div className="space-y-4 text-sm text-gray-600 leading-relaxed">
            ...
            <Link to="/people" className="inline-block text-xs tracking-widest text-gray-900 hover:underline mt-2">
              ABOUT US →
            </Link>
          </div>
        </div>
      ...
      <section className="border-t border-gray-100 bg-[#faf9f7]">
        <div className="max-w-7xl mx-auto px-6 sm:px-12 py-14">
          <div className="flex items-center justify-between mb-8">
            ...
            <Link to="/media" className="text-xs tracking-widest text-gray-500 hover:text-gray-900 transition-colors">
              VIEW ALL →
            </Link>
          </div>
          <div className="divide-y divide-gray-200">
            {newsItems.slice(0, 3).map((item) => (
              <Link key={item.id} to={`/media/news/${item.slug}`} className="flex gap-6 sm:gap-10 py-6 group">
                <div className="w-24 flex-shrink-0 text-xs text-gray-400 pt-0.5">{item.date}</div>
      ...
      <section className="border-t border-gray-100">
        <div className="max-w-7xl mx-auto px-6 sm:px-12 py-14">
          <div className="flex items-center justify-between mb-10">
            ...
            <Link to="/archive" className="text-xs tracking-widest text-gray-500 hover:text-gray-900 transition-colors">
              VIEW ALL →
            </Link>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
            {events.map((event) => (
              <Link key={event.id} to={`/archive/${event.id}`} className="group">
      ...
      <section className="border-t border-gray-100 bg-[#faf9f7]">
        <div className="max-w-7xl mx-auto px-6 sm:px-12 py-14">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {blogPosts.map((post) => (
              <Link key={post.id} to={`/media/blog/${post.slug}`} className="group">
                {post.image && (
