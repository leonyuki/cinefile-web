...
import { FileText, Image as ImageIcon, Calendar, Building2, PlusCircle, Edit3, Loader2, Eye } from 'lucide-react';
import ReactMarkdown from 'react-markdown'; // 🌟 追加: マークダウンの描画用
...
export default function EventsTab({ availableEvents, availablePartners, refreshMasterData }: Props) {
  ...
  const renderPartnerSelector = (label: string, selectedList: string[], setList: React.Dispatch<React.SetStateAction<string[]>>, fieldName: string) => (
    <div className="space-y-2">
      ...
      <div className="border border-gray-200 rounded-sm p-3 max-h-32 overflow-y-auto space-y-2 bg-white">
        {availablePartners.length > 0 ? (
          availablePartners.map(partner => (
            <label key={partner.id} className="flex items-center gap-2 cursor-pointer group">
              ...
              />
              <span className="text-xs font-medium text-gray-800 group-hover:text-gray-500 transition-colors">{partner.name}</span>
            </label>
          ))
        ) : (
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8 animate-in fade-in duration-300">
      <div className="flex gap-4 border-b border-gray-100 pb-4">
        <button
          type="button"
          onClick={() => { setEventMode('create'); setStatus(''); }}
        ...
        <button
          type="button"
          onClick={() => { setEventMode('edit'); setStatus(''); }}
          className={`flex items-center gap-1.5 text-xs tracking-widest font-medium py-1.5 px-3 rounded-xs transition-colors ${
      ...
      {eventMode === 'edit' && (
        <div className="animate-in slide-in-from-top-2 duration-200">
          ...
          <select
            ...
            onChange={(e) => setSelectedEditEventId(e.target.value)}
            required
      ...
      <div className="grid sm:grid-cols-[2fr_1fr] gap-6">
        <div>
          ...
          <input type="text" name="title" value={eventTitle || ''} onChange={(e) => setEventTitle(e.target.value)} disabled={isLoadingData} required className="w-full border border-gray-200 p-3 text-sm focus:outline-none focus:border-gray-900 rounded-sm" placeholder="イベント名を入力" />
        </div>
        <div>
          <label className="block text-[10px] tracking-widest text-gray-400 uppercase mb-2 font-semibold">サブタイトル / 英題</label>
          <input type="text" name="subtitle" value={eventSubtitle || ''} onChange={(e) => setEventSubtitle(e.target.value)} disabled={isLoadingData} className="w-full border border-gray-200 p-3 text-sm focus:outline-none focus:border-gray-900 rounded-sm" />
        </div>
      </div>
      ...
      <div className="p-5 border border-dashed border-gray-200 bg-gray-50 rounded-sm">
        ...
        <input type="file" name="image" accept="image/*" required={eventMode === 'create'} className="text-xs text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-sm file:border-0 file:text-xs file:font-medium file:bg-gray-900 file:text-white hover:file:opacity-80 file:cursor-pointer" />
      </div>
      ...
      <div className="p-6 border border-gray-100 bg-[#faf9f7] rounded-sm space-y-6">
        ...
        <div className="grid sm:grid-cols-2 gap-6">
          <div>
            ...
            <input type="number" name="year" value={eventYear || ''} onChange={(e) => setEventYear(Number(e.target.value))} disabled={isLoadingData} required className="w-full bg-white border border-gray-200 p-3 text-sm focus:outline-none focus:border-gray-900 rounded-sm" />
          </div>
          <div>
            <label className="block text-[10px] tracking-widest text-gray-400 uppercase mb-2">都市名（英語表記）</label>
            <input type="text" name="city" value={eventCity || ''} onChange={(e) => setEventCity(e.target.value)} disabled={isLoadingData} placeholder="例: Tokyo, Copenhagen" className="w-full bg-white border border-gray-200 p-3 text-sm focus:outline-none focus:border-gray-900 rounded-sm" />
          </div>
        </div>
        <div className="grid sm:grid-cols-2 gap-6">
          <div>
            <label className="block text-[10px] tracking-widest text-gray-400 uppercase mb-2">開催期間（表示用テキスト）</label>
            <input type="text" name="date" value={eventDate || ''} onChange={(e) => setEventDate(e.target.value)} disabled={isLoadingData} required className="w-full bg-white border border-gray-200 p-3 text-sm focus:outline-none focus:border-gray-900 rounded-sm" />
          </div>
          <div>
            <label className="block text-[10px] tracking-widest text-gray-400 uppercase mb-2">具体的な開催場所</label>
            <input type="text" name="location" value={eventLocation || ''} onChange={(e) => setEventLocation(e.target.value)} disabled={isLoadingData} required className="w-full bg-white border border-gray-200 p-3 text-sm focus:outline-none focus:border-gray-900 rounded-sm" />
          </div>
        </div>
        <div>
          <label className="block text-[10px] tracking-widest text-gray-400 uppercase mb-2">ステータス</label>
          <select name="status" value={eventStatus || 'Past'} onChange={(e) => setEventStatus(e.target.value)} disabled={isLoadingData} className="w-full bg-white border border-gray-200 p-3 text-sm focus:outline-none focus:border-gray-900 rounded-sm cursor-pointer">
      ...
      <div className="space-y-4">
        <div>
          ...
          <textarea 
            ...
            required 
            rows={15} 
            className="w-full border border-gray-200 p-4 text-sm font-mono bg-[#faf9f7] focus:bg-white focus:outline-none focus:border-gray-900 rounded-sm transition-colors resize-y leading-relaxed" 
      ...
      <button type="submit" disabled={isLoading || isLoadingData} className="w-full py-4 bg-gray-950 text-white text-xs tracking-widest uppercase hover:bg-gray-800 transition-colors rounded-sm font-semibold disabled:opacity-50">
        {(isLoading || isLoadingData) ? <Loader2 className="w-4 h-4 animate-spin mx-auto" /> : eventMode === 'edit' ? 'イベント情報を修正・上書きする' : 'events として登録・投稿する'}
      </button>
